...
class TestSemantics {
  ...
  TestSemantics({
    ...
    this.scrollIndex,
    this.scrollChildren,
    final Iterable<SemanticsTag>? tags,
  ...
  static Matrix4 _applyRootChildScale(final Matrix4? transform) {
  ...
  bool _matches(
    ...
    final bool ignoreRect = false,
    final bool ignoreTransform = false,
    final bool ignoreId = false,
    final DebugSemanticsDumpOrder childOrder =
        DebugSemanticsDumpOrder.inverseHitTest,
  }) {
    bool fail(final String message) {
    ...
    final int flagsBitmask = flags is int
        ...
        : (flags as List<SemanticsFlag>).fold<int>(
            0,
            (final int bitmask, final SemanticsFlag flag) =>
                bitmask | flag.index,
          );
    ...
    final int actionsBitmask = actions is int
        ...
        : (actions as List<SemanticsAction>).fold<int>(
            0,
            (final int bitmask, final SemanticsAction action) =>
                bitmask | action.index,
          );
  ...
  @override
  String toString([final int indentAmount = 0]) {
    ...
    if (transform != null) {
      buf.writeln(
        '$indent  transform:\n${transform.toString().trim().split('\n').map<String>((final String line) => '$indent    $line').join('\n')},',
      );
    }
    if (elevation != null) {
...
class SemanticsTester {
  ...
  bool _stringAttributesEqual(
    final List<StringAttribute> first,
    final List<StringAttribute> second,
  ) {
  ...
  Iterable<SemanticsNode> nodesWith({
    ...
    final SemanticsNode? ancestor,
  }) {
    bool checkNode(final SemanticsNode node) {
      ...
      if (actions != null) {
        final int expectedActions = actions.fold<int>(
          0,
          (final int value, final SemanticsAction action) =>
              value | action.index,
        );
      ...
      if (flags != null) {
        final int expectedFlags = flags.fold<int>(
          0,
          (final int value, final SemanticsFlag flag) => value | flag.index,
        );
    ...
    bool visit(final SemanticsNode node) {
      if (checkNode(node)) {
        result.add(node);
      }
  ...
  static String _tagsToSemanticsTagExpression(final Set<SemanticsTag> tags) {
    return '<SemanticsTag>[${tags.map<String>((final SemanticsTag tag) => "const SemanticsTag('${tag.name}')").join(', ')}]';
  }
  ...
  static String _generateSemanticsTestForNode(
    ...
    return buf
        .toString()
        .split('\n')
        .map<String>((final String l) => '$indent$l')
        .join('\n');
  }
}
...
class _HasSemantics extends Matcher {
  ...
  @override
  bool matches(
    covariant final SemanticsTester item,
    final Map<dynamic, dynamic> matchState,
  ) {
  ...
  String _indent(final String? text) {
    return text
        ...
        .trimRight()
        .split('\n')
        .map<String>((final String line) => '  $line')
        .join('\n');
  }
