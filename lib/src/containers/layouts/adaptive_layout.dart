...
///    from the top to the top of [bottomNavigation]. Must have defined size.
...
///
/// [ArnaAdaptiveLayout] handles the placement of the slots on the app window
/// and animations regarding their macromovements.
///
/// See also:
///
...
///    [SlotLayout].
///  * [ArnaAdaptiveScaffold], which provides a more friendly API with less
///    customizability. and holds a preset of animations and helper builders.
class ArnaAdaptiveLayout extends StatefulWidget {
  /// Creates a const [ArnaAdaptiveLayout] widget.
  const ArnaAdaptiveLayout({
    super.key,
  ...
  ///
  /// The beginning side means the left when the ambient [Directionality] is
  /// [TextDirection.ltr] and on the right when it is [TextDirection.rtl].
  ///
  ...
  /// Whether or not the developer wants the smooth entering slide transition
  /// on [secondaryBody].
  ///
  /// Defaults to true.
...
class _ArnaAdaptiveLayoutState extends State<ArnaAdaptiveLayout>
  ...
  Widget build(BuildContext context) {
    ...
    return CustomMultiChildLayout(
      delegate: _ArnaAdaptiveLayoutDelegate(
        slots: slots,
...
/// positions.
class _ArnaAdaptiveLayoutDelegate extends MultiChildLayoutDelegate {
  _ArnaAdaptiveLayoutDelegate({
    required this.slots,
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(_SlotIds.primaryNavigation.name)) {
      ...
      if (textDirection) {
        ...
        positionChild(
          _SlotIds.primaryNavigation.name,
          Offset(size.width - currentSize.width, 0),
        );
    ...
    if (hasChild(_SlotIds.body.name) && hasChild(_SlotIds.secondaryBody.name)) {
      ...
      if (!textDirection &&
        ...
        if (hinge != null) {
          positionChild(
            _SlotIds.body.name,
            Offset(currentSBodySize.width + leftMargin + hingeWidth, 0),
          );
          positionChild(_SlotIds.secondaryBody.name, Offset(leftMargin, 0));
        } else {
          positionChild(
            _SlotIds.body.name,
            Offset(currentSBodySize.width + leftMargin, 0),
          );
          positionChild(_SlotIds.secondaryBody.name, Offset(leftMargin, 0));
        }
      } else {
  ...
  @override
  bool shouldRelayout(_ArnaAdaptiveLayoutDelegate oldDelegate) {
